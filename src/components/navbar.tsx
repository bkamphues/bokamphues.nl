...
function NavigationLinks() {
  ...
  let links = data.allSiteSettingsYaml.edges.filter(setting => setting.node.nav_links != null)[0].node.nav_links

  // validate each URL and return html
  let navigationLinks = []
  links.forEach(link => {
    if (validURL(link.link_url)) {
      navigationLinks.push(<a href={link.link_url} target="_blank" rel="noopener noreferrer">{link.link_name}</a>)
    } else {
